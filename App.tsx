...
const App = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col transition-colors duration-300 font-sans pb-[env(safe-area-inset-bottom)]">
      ...
      <header className="px-3 py-2.5 flex items-center justify-between sticky top-0 z-[1000] glass border-b border-white dark:border-slate-800/80 shadow-sm pt-[calc(0.5rem+env(safe-area-inset-top))]">
        <div className="flex items-center gap-1.5 sm:gap-2.5 min-w-0 flex-1">
          ...
          <div className="relative min-w-0">
            <button onClick={() => setShowBranchDropdown(!showBranchDropdown)} className="flex items-center gap-1.5 px-2.5 py-1.5 bg-white dark:bg-slate-800/80 rounded-xl border border-slate-200 dark:border-slate-700 active-scale transition-all shadow-sm max-w-full">
              ...
              <span className="text-[10px] sm:text-[11px] font-black uppercase dark:text-white truncate max-w-[80px] sm:max-w-[140px] tracking-tight">{currentBranchName}</span>
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 ml-2 shrink-0">
          ...
          <button onClick={() => setIsDark(!isDark)} className="w-9 h-9 sm:w-10 sm:h-10 glass rounded-xl border border-slate-200 dark:border-slate-700 flex items-center justify-center active-scale shadow-sm">
             {isDark ? <Sun className="w-4 h-4 text-amber-500" /> : <Moon className="w-4 h-4 text-brand-600" />}
          </button>
          <button onClick={() => setConfirmModal({ show: true, title: t('logout'), message: t('confirm_logout'), onConfirm: () => { localStorage.removeItem('tokymon_user'); setCurrentUser(null); } })} className="w-9 h-9 sm:w-10 sm:h-10 bg-rose-600 text-white rounded-xl shadow-lg active-scale flex items-center justify-center border border-rose-500/50"><LogOut className="w-4 h-4" /></button>
        </div>
      </header>
        ...
        {activeTab === 'settings' && (
          <div className="space-y-6">
            ...
            <div className="bg-white/95 dark:bg-slate-900/90 backdrop-blur-md rounded-[2rem] p-6 sm:p-8 border border-white dark:border-slate-800 shadow-soft min-h-[450px] animate-ios">
                ...
                {/* Fixed reference from setGlobalConfirm to setConfirmModal */}
      ...
      <div className="fixed bottom-0 left-0 right-0 px-5 z-[2000] flex justify-center pointer-events-none ios-bottom-nav">
        <nav className="h-18 sm:h-20 max-w-md w-full glass border border-white dark:border-slate-800/80 flex items-center justify-around px-2 rounded-[2.2rem] shadow-2xl pointer-events-auto">
          {[
            ...
            <button key={tab.id} onClick={() => { setActiveTab(tab.id as any); window.scrollTo({ top: 0, behavior: 'smooth' }); }} className={`flex flex-col items-center gap-1.5 relative py-2 rounded-2xl flex-1 transition-all active-scale ${activeTab === tab.id ? 'text-brand-600 dark:text-brand-400' : 'text-slate-500 dark:text-slate-500'}`}>
              <tab.icon className={`w-5.5 h-5.5 sm:w-6 sm:h-6 ${activeTab === tab.id ? 'stroke-[2.5]' : 'stroke-[2]'}`} />
              <span className={`text-[9px] sm:text-[10px] font-black uppercase tracking-tight ${activeTab === tab.id ? 'opacity-100' : 'opacity-60'}`}>{tab.label}</span>
