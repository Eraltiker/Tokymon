    ...
    return (
      <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center p-8 relative overflow-hidden font-sans">
        ...
        <div className="absolute top-8 right-8 z-50">
           <button onClick={toggleLanguage} className="px-5 py-3 glass rounded-2xl border border-white/10 text-white active:scale-95 transition-all flex items-center gap-3 shadow-2xl">
        ...
        <div className="w-full max-w-[420px] z-10 space-y-12">
          <div className="text-center space-y-8 animate-in fade-in slide-in-from-top-4 duration-1000">
            <div className="relative inline-block group">
              {data.logoUrl ? (
                <div className="relative p-6 bg-white/5 rounded-[3.5rem] backdrop-blur-2xl border border-white/10 shadow-2xl active:scale-95 transition-transform">
                  <img src={data.logoUrl} alt="Tokymon" className="w-32 h-32 mx-auto object-contain" />
                </div>
              ) : (
                <div className="w-32 h-32 bg-brand-600 rounded-[3.5rem] flex items-center justify-center shadow-vivid mx-auto transform rotate-6 active:rotate-0 transition-all duration-500 relative overflow-hidden">
            ...
            <div className="space-y-3">
              ...
              <p className="text-[11px] font-black text-brand-400 uppercase tracking-[0.6em] ml-1">Finance Core</p>
            </div>
          </div>
          <form onSubmit={(e) => {
            e.preventDefault();
          ...
          }} className="glass p-10 rounded-[3.5rem] border border-white/10 shadow-2xl space-y-8 relative overflow-hidden group">
            <div className="space-y-6">
              <div className="space-y-2.5">
                <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-2">Access ID</label>
                <div className="relative group/input">
                  <input type="text" value={loginForm.username} onChange={e => setLoginForm({...loginForm, username: e.target.value})} className="w-full p-5 bg-white/5 rounded-[1.8rem] font-bold border-2 border-transparent focus:border-brand-500 outline-none text-white transition-all pl-14" placeholder="Username" required />
                  <UserCircle2 className="absolute left-5 top-1/2 -translate-y-1/2 w-6 h-6 text-slate-500" />
                </div>
              </div>
              <div className="space-y-2.5">
                <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-2">Secure Key</label>
                <div className="relative group/input">
                  <input type="password" value={loginForm.password} onChange={e => setLoginForm({...loginForm, password: e.target.value})} className="w-full p-5 bg-white/5 rounded-[1.8rem] font-bold border-2 border-transparent focus:border-brand-500 outline-none text-white transition-all pl-14" placeholder="••••••••" required />
            ...
            <button type="submit" className="w-full py-6 bg-brand-600 hover:bg-brand-500 text-white rounded-[2rem] font-black uppercase text-sm tracking-widest active:scale-95 transition-all flex items-center justify-center gap-4 shadow-vivid">
    ...
    <div className="min-h-screen flex flex-col transition-colors duration-300 font-sans pb-[env(safe-area-inset-bottom)]">
      {confirmModal && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-6">
          ...
          <div className="bg-white dark:bg-slate-900 w-full max-w-xs rounded-[3rem] p-10 relative z-10 border border-slate-100 dark:border-slate-800 shadow-2xl animate-in zoom-in-95">
            <div className="w-16 h-16 bg-rose-100 dark:bg-rose-900/30 rounded-3xl flex items-center justify-center text-rose-600 mb-6 mx-auto">
              <AlertTriangle className="w-8 h-8" />
            </div>
            <h3 className="text-lg font-black text-slate-900 dark:text-white text-center uppercase mb-3 tracking-tight">{confirmModal.title}</h3>
            <p className="text-[11px] font-bold text-slate-400 text-center mb-10 leading-relaxed uppercase">{confirmModal.message}</p>
            <div className="flex gap-3">
              <button onClick={() => setConfirmModal(null)} className="flex-1 py-5 bg-slate-100 dark:bg-slate-800 text-slate-500 rounded-2xl font-black uppercase text-[10px] tracking-widest active:scale-95 transition-all">{t('cancel')}</button>
              <button onClick={() => { confirmModal.onConfirm(); setConfirmModal(null); }} className="flex-1 py-5 bg-brand-600 text-white rounded-2xl font-black uppercase text-[10px] tracking-widest active:scale-95 transition-all">{t('save')}</button>
            </div>
          </div>
        </div>
      )}
