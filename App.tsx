...
const App = () => {
  ...
  if (!currentUser) {
    return (
      <div className="min-h-screen bg-white dark:bg-slate-950 flex flex-col items-center justify-center p-6 overflow-hidden relative">
        ...
        }} className="w-full max-w-[320px] space-y-4 relative z-10">
          <div className="text-center space-y-1 mb-8">
             <h1 className="text-3xl font-black dark:text-white uppercase tracking-tighter">Tokymon</h1>
             <p className="text-[10px] font-black text-indigo-500 uppercase tracking-widest">Admin Control Center</p>
          </div>
          
          <div className="space-y-3">
            <div className="relative group">
              <input type="text" placeholder="Username" value={loginForm.username} onChange={e => setLoginForm({...loginForm, username: e.target.value})} className="w-full p-4 bg-slate-50 dark:bg-slate-900 rounded-2xl font-bold border-2 border-transparent focus:border-indigo-500 outline-none dark:text-white transition-all pl-12" />
              <Users className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 group-focus-within:text-indigo-500" />
            </div>
            <div className="relative group">
              <input type="password" placeholder="Password" value={loginForm.password} onChange={e => setLoginForm({...loginForm, password: e.target.value})} className="w-full p-4 bg-slate-50 dark:bg-slate-900 rounded-2xl font-bold border-2 border-transparent focus:border-indigo-500 outline-none dark:text-white transition-all pl-12" />
              <Fingerprint className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 group-focus-within:text-indigo-500" />
            </div>
          </div>

          {loginError && <p className="text-rose-500 text-center text-[9px] font-black uppercase bg-rose-50 dark:bg-rose-900/20 py-2 rounded-xl">{loginError}</p>}
          
          <button type="submit" className="w-full py-4 bg-indigo-600 text-white rounded-2xl font-black uppercase shadow-xl active:scale-95 transition-all text-xs tracking-widest">
            Truy cập
          </button>
        </form>
      </div>
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex flex-col transition-colors duration-300 pb-20">
      <header className="px-4 py-3 flex items-center justify-between sticky top-0 z-[100] glass border-b border-white/20 dark:border-slate-800/50 shadow-sm">
        <div className="flex items-center gap-2">
          <div className="relative group">
            <div className="flex items-center gap-1.5 px-3 py-1.5 bg-white dark:bg-slate-900 rounded-xl shadow-sm border dark:border-slate-800 transition-all cursor-pointer active:scale-95">
              <div className="w-1.5 h-1.5 rounded-full bg-indigo-500 animate-pulse" />
              <span className="text-[10px] font-black uppercase dark:text-white truncate max-w-[80px]">{currentBranch?.name || 'Cơ sở'}</span>
              <ChevronDown className="w-3 h-3 text-slate-400" />
            </div>
            <div className="absolute top-full left-0 mt-2 w-56 bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl rounded-2xl shadow-2xl border dark:border-slate-800 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-[200] overflow-hidden">
              {allowedBranches.map(b => (
                <div key={b.id} onClick={() => { setCurrentBranchId(b.id); localStorage.setItem('tokymon_current_branch', b.id); }} className="px-4 py-3 cursor-pointer hover:bg-indigo-600 hover:text-white transition-colors flex items-center justify-between">
                  <span className="text-[9px] font-black uppercase">{b.name}</span>
                  {currentBranchId === b.id && <ShieldCheck className="w-3 h-3 text-emerald-500" />}
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="flex items-center gap-2">
          <div className={`flex items-center gap-2 bg-white/50 dark:bg-slate-900/50 px-2 py-1.5 rounded-xl border ${syncStatus === 'ERROR' ? 'border-rose-500' : 'dark:border-slate-800'}`}>
             ...
             <span className={`text-[8px] font-black uppercase leading-none ${syncStatus === 'ERROR' ? 'text-rose-500' : 'text-slate-400 dark:text-slate-500'}`}>{lastSyncTime || 'Ready'}</span>
          </div>
          
          <button onClick={() => setIsDark(!isDark)} className="p-2 bg-white dark:bg-slate-900 rounded-xl shadow-sm border dark:border-slate-800 active:scale-90">
          ...
          <div className="space-y-6 pb-6">
            <div className="flex gap-2 overflow-x-auto no-scrollbar pb-1">
              {[
                ...
                <button key={sub.id} onClick={() => setSettingsSubTab(sub.id as any)} style={{ display: (sub.id === 'branches' || sub.id === 'users') && (currentUser.role !== UserRole.SUPER_ADMIN) ? 'none' : 'flex' }} className={`px-4 py-2.5 rounded-xl text-[9px] font-black uppercase tracking-wider border transition-all flex items-center gap-2 shrink-0 ${settingsSubTab === sub.id ? 'bg-indigo-600 border-indigo-600 text-white shadow-lg shadow-indigo-200' : 'bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 text-slate-500'}`}>
                  <sub.icon className="w-3.5 h-3.5" /> {sub.label}
                </button>
              ))}
            </div>

            <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl rounded-[2rem] p-5 border dark:border-slate-800 shadow-sm min-h-[400px]">
              {settingsSubTab === 'sync' && (
                <div className="space-y-6 animate-in fade-in duration-300">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h3 className="text-sm font-black dark:text-white uppercase leading-none mb-1">Cloud ID</h3>
                      <p className="text-[8px] font-bold text-slate-400 uppercase tracking-wider">Đồng bộ đa thiết bị</p>
                    </div>
                  </div>
                  ...
                  <div className="space-y-4">
                    <input 
                      ...
                      placeholder="KVDB ID..." 
                      className="w-full p-4 bg-slate-50 dark:bg-slate-800 rounded-xl font-black border-2 border-transparent focus:border-indigo-500 outline-none text-xs dark:text-white" 
                    />

                    <button onClick={() => handleCloudSync()} disabled={isSyncing} className="w-full py-4 bg-indigo-600 text-white rounded-xl font-black uppercase text-[10px] tracking-widest flex items-center justify-center gap-3 active:scale-95 disabled:opacity-50 shadow-lg shadow-indigo-100 dark:shadow-none">
                ...
                <div className="space-y-4">
                  <div className="space-y-3 overflow-y-auto max-h-[500px] no-scrollbar">
                    {data.auditLogs.slice().reverse().map(log => (
                      <div key={log.id} className="p-3 bg-slate-50 dark:bg-slate-800/50 rounded-xl border dark:border-slate-800 flex items-start gap-3">
                        <div className="w-7 h-7 bg-indigo-100 dark:bg-indigo-900/30 rounded-lg flex items-center justify-center text-indigo-600 shrink-0 font-black text-[9px] uppercase">{log.username[0]}</div>
                        <div className="flex-1 min-w-0">
                          <div className="flex justify-between items-center mb-0.5">
                            <span className="text-[8px] font-black text-indigo-500 uppercase">{log.username}</span>
                            <span className="text-[7px] font-bold text-slate-400">{new Date(log.timestamp).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</span>
                          </div>
                          <p className="text-[10px] font-bold text-slate-700 dark:text-slate-200 truncate">{log.details}</p>
                        </div>
                      </div>
                    ))}
                  </div>
            ...
            <button onClick={() => { if(confirm("Đăng xuất?")){localStorage.removeItem('tokymon_user'); setCurrentUser(null);} }} className="w-full py-4 bg-rose-500/5 dark:bg-rose-500/10 text-rose-500 rounded-2xl font-black uppercase text-[9px] tracking-widest flex items-center justify-center gap-2 border border-rose-500/10 active:scale-95">
      ...
      <nav className="fixed bottom-3 left-4 right-4 h-16 bg-white/80 dark:bg-slate-900/80 backdrop-blur-2xl border dark:border-slate-800 flex items-center justify-around px-2 rounded-2xl shadow-xl z-[200]">
        {[
          ...
          <button 
            ...
            className={`flex flex-col items-center gap-0.5 transition-all relative px-3 py-1.5 rounded-xl ${activeTab === tab.id ? 'text-indigo-600' : 'text-slate-400'}`}
          >
            {activeTab === tab.id && <div className="absolute inset-0 bg-indigo-500/5 rounded-xl -z-10" />}
            <tab.icon className={`w-5 h-5 ${activeTab === tab.id ? 'stroke-[3]' : 'stroke-[2]'}`} />
            <span className={`text-[7px] font-black uppercase tracking-tight ${activeTab === tab.id ? 'opacity-100' : 'opacity-60'}`}>{tab.label}</span>
          </button>
        ))}
      </nav>
    </div>
