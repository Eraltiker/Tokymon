...
import { Transaction, TransactionType, formatCurrency, HistoryEntry, Language } from '../types';
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-end sm:items-center justify-center p-0 sm:p-4 animate-in fade-in duration-300">
      ...
      <div className="bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl w-full max-w-lg rounded-t-[2rem] sm:rounded-3xl shadow-ios relative z-[201] overflow-hidden flex flex-col max-h-[90vh] animate-ios">
        <div className="w-10 h-1 bg-slate-200 dark:bg-slate-800 rounded-full mx-auto my-3 sm:hidden" />
        <div className="px-5 py-3 border-b border-white/10 dark:border-slate-800 flex justify-between items-center bg-slate-50/50 dark:bg-slate-800/30">
          <div>
            <h3 className="font-black text-slate-800 dark:text-white uppercase tracking-tight text-xs">{t('edit_title')}</h3>
            <p className="text-[8px] font-bold text-slate-400 uppercase tracking-widest">{category}</p>
          </div>
          <button onClick={onClose} className="p-1.5 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-full transition-colors text-slate-500"><X className="w-5 h-5" /></button>
        </div>
        <div className="p-5 overflow-y-auto no-scrollbar space-y-4">
          <div className="flex items-center gap-1.5 bg-slate-50 dark:bg-slate-950/50 p-1.5 rounded-2xl border border-white/10 dark:border-slate-800">
            ...
            <div className="flex-1 text-center relative flex flex-col items-center">
               <span className="text-[10px] font-black dark:text-white uppercase flex items-center gap-1.5"><Calendar className="w-3 h-3 text-brand-500" /> {formatDateDisplay(date)}</span>
          ...
          {transaction.type === TransactionType.INCOME ? (
            <div className="space-y-4">
              <div className="space-y-1">
                ...
                <input type="text" inputMode="decimal" value={kasseInput} onChange={e => validateAndSetAmount(e.target.value, setKasseInput)} className="w-full px-5 py-3 bg-brand-50/10 dark:bg-brand-900/10 border border-brand-100 dark:border-brand-900 rounded-2xl font-black text-xl text-brand-600 outline-none" />
              </div>
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="space-y-1">
                  ...
                  <input type="text" inputMode="decimal" value={cardTotalInput} onChange={e => validateAndSetAmount(e.target.value, setCardTotalInput)} className="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-800 border dark:border-slate-700 rounded-xl font-black text-sm text-brand-500" />
                </div>
              </div>
            </div>
          ) : (
            <div className="space-y-1">
              ...
              <input type="text" inputMode="decimal" value={expenseAmount} onChange={e => validateAndSetAmount(e.target.value, setExpenseAmount)} className="w-full py-3.5 bg-rose-50/10 dark:bg-rose-900/10 border border-rose-100 dark:border-rose-900 rounded-2xl font-black text-xl text-rose-600 outline-none text-center" />
            </div>
          )}
          <div className="bg-slate-50 dark:bg-slate-950/30 p-4 rounded-2xl border border-white/10 shadow-inner">
            <textarea value={note} onChange={e => setNote(e.target.value)} placeholder={t('note')} className="w-full bg-transparent text-xs font-bold outline-none dark:text-white resize-none h-20" />
          </div>
