...
const EditTransactionModal: React.FC<EditTransactionModalProps> = ({ 
  ...
  const smartSuggestions = useMemo(() => {
    if (isIncome) return ["Điều chỉnh Z-Bon", "Sai lệch Thẻ", "Sai lệch App", "Đối soát lại"];
    if (category.includes("Lương")) return ["Hoàn ứng", "Trả nợ lương", "Truy thu lương"];
    if (category.includes("Nợ")) return ["Trả nợ nhà cung cấp", "Thanh toán từng phần", "Dứt điểm công nợ"];
    return ["Điều chỉnh số liệu", "Bổ sung ghi chú", "Thay đổi nguồn chi"];
  }, [isIncome, category]);
  ...
  const handleSave = () => {
    ...
    if (newNote.trim()) updatedNotes.push(newNote.trim());
        ...
        <div className="p-6 overflow-y-auto no-scrollbar space-y-6 pb-10">
          ...
          <div className="space-y-4">
             <div className="flex justify-between items-center px-1">
                ...
                <button type="button" onClick={() => handleAddNote()} disabled={!newNote.trim()} className="text-[8px] font-black text-brand-600 uppercase flex items-center gap-1 bg-brand-50 dark:bg-brand-900/30 px-3 py-1.5 rounded-xl border border-brand-100 dark:border-brand-800"><Plus className="w-3 h-3" /> Thêm mới</button>
             </div>

             {/* Gợi ý thông minh trong Modal */}
             <div className="flex gap-2 overflow-x-auto no-scrollbar py-1">
                {smartSuggestions.map((s, i) => (
                  <button 
                    ...
                    className="shrink-0 px-4 py-2 bg-slate-100 dark:bg-slate-800 border dark:border-slate-700 rounded-full text-[9px] font-black text-slate-500 uppercase tracking-widest active-scale flex items-center gap-1.5"
                  >
             ...
             <div className="relative">
                <input 
                  ...
                  className="w-full p-4 bg-white dark:bg-slate-950 border-2 border-slate-100 dark:border-slate-800 rounded-2xl text-[12px] font-bold outline-none focus:border-brand-500 shadow-sm"
                />
             </div>
